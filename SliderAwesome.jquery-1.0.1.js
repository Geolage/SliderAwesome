...
(function($) {
    var SliderAwesome = function(element, options){
        ...
        try{
            var _effect=options.effect , _styles=$.fn.sliderAwesome.defaults.styles,
        ...
        function effectSetter(direction){
            switch(settings.effect){
                ...
                case 'gridsShuffleUp':{
                    vars.currentEffect='gridsShuffleUp';
                    break;
                }
            }
        }
        ...
        if(settings.keyboardNav){
            ...
            $(window).on({
                keydown:function(e){
                    ...
                    if(code_arr.indexOf(code)==-1){ code_arr.push(code) };
                    if (code_arr.length>1) {return false};
                    switch(code_arr[0]){
                        case 37:{
                            if(vars.actionPause){
                                return false;
        ...
        if(settings.clickNav){
            ...
            $(slider).on('click', '.sa-control-pre', function(e){
                ...
                if(vars.actionPause&&vars.running) { return false; }
                clearInterval(timer);
                timer = '';
        ...
        var effectRun = function(slider, imgs, settings, control){          
            ...
            var styles=[],currentEffect = vars.currentEffect.trim() || settings.effect;
            ...
            if(settings.styles.slice(0,3).indexOf(currentEffect)!==-1){
                ...
                if(currentEffect === 'flickrSlicesRight') { slices = $('.sa-img-slice', slider); }
                ...
                if(currentEffect === 'shutters' || currentEffect === 'shuttersRight'){
                    ...
                    $('.sa-img-slice', slider).each(function(){
                        ...
                        if(i === settings.slices-1){
                            setTimeout(function(){
                                slice.animate({ width:origWidth, opacity:'1.0' }, settings.speed*1.1, '', function(){ slider.trigger('sa:animFinished'); });
                            }, (100 + timeBuff));
                        } else {
                            setTimeout(function(){
                                slice.animate({ width:origWidth, opacity:'1.0' }, settings.speed*1.1);
                            }, (100 + timeBuff));
                        }
                        timeBuff += 50;
                        i++;
                    ...
                    for(var j=i;j>=0;j--){
                        ...
                        $slice.css({opacity:'1',top:'0px'});
                        (function(slice){
                            if(j === 0){
                                setTimeout(function(){
                                    slice.animate({ width:'0px', opacity:'0' }, settings.speed*1.1, '', function(){ slider.trigger('sa:animFinished'); });
                                }, (100 + timeBuff));
                            } else{
                                setTimeout(function(){
                                    slice.animate({ width:'0px', opacity:'0' }, settings.speed*1.1);
                                }, (100 + timeBuff));
                            }
                        })($slice)
                        timeBuff += 50;
                ...
                if(currentEffect === 'scrollLeft'){
                    firstSlice.css({'width': slider.width() + 'px','opacity': '1'}).children('img:first').attr('src',$('img.sa-main-image',slider).attr('src')).siblings('.sa-img-slice').remove();
                    ...
                    anim_css={width: slider.width() + 'px'};  
                }
                firstSlice.animate(anim_css, (settings.speed*2.2), '', function(){ slider.trigger('sa:animFinished'); });
                ...
                createSlices(slider, settings, vars);
                firstSlice = $('.sa-img-slice:first', slider);
                var anim_css = (currentEffect !== 'takeOutLeft') ? {right:-slider.width()} : {right:slider.width()};
                firstSlice.children('img').attr('src',$('img.sa-main-image',slider).attr('src')).end().css({
                    'width': slider.width(),
                    'opacity': '1',
                    'left': '',
                    'right': '0px'
                }).siblings('.sa-img-slice').remove().end().animate(anim_css,(settings.speed*2),'',function(){slider.trigger('sa:animFinished');}); 
                $('img.sa-main-image',slider).attr('src',vars.currentImage.attr('src'));
                        ...
                        for(var rows = 0; rows < settings.boxRows; rows++){
                            if(preCol >= 0 && preCol < settings.boxCols){
                                (function(row, col, time, i, totalBoxes) {
                                    ...
                                    var w = box.width();
                                    var h = box.height();
                                    var box_css={opacity:'1', width:w, height:h};
                                    if(currentEffect === 'boxesRain' || currentEffect === 'boxesRainLeft' || currentEffect === 'boxesRainRight' || currentEffect === 'gridsShuffleUpLeft'){
                                        ...
                                        box.css('opacity','1');
                                        box_css={opacity:'0', width:'auto',height:'auto'};
                                    }
                                    if(i === totalBoxes-1){
                                        setTimeout(function(){
